...
class Game {
    constructor (teamA, teamB, teamC, teamD) { // only four teams in the league
...
class Menu {
    ...
    showMainMenuOptions() {
        return prompt(`MAIN MENU
            ...
            2) Add a New Player to the League
            3) Assign a Player to a team
            `);
    }
    ...
    addPlayer () {
        ...
        this.leaguePlayers.push(newPlayer); // add new player to array league players
        alert(`${newPlayer.name}, ${newPlayer.phone}, ${newPlayer.position}, has been added to the league.`);
        }
    ...
    assignPlayer() {
        ...
        this.selectedTeam.teamRoster.push(playerToAssign);
...
for (let i = 0; i < 6; i++) {
    teamA.addPlayer(leaguePlayers[i]);
}
/*
...
for (let i = 0; i < 6; i++) {
    teamB.addPlayer(leaguePlayers[i+6]);
}
/*
...
for (let i = 0; i < 6; i++) {
    teamC.addPlayer(leaguePlayers[i+12]);
}
/*
...
for (let i = 0; i < 6; i++) {
    teamD.addPlayer(leaguePlayers[i+18]);
}
/*
